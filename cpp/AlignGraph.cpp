...
//**********************************************************************************
//
// Modified: Ilja Pljusnin, University of Helsinki
...
void parseDelta(string buf, int & a, int & b, int & c, int & d, int & realA, int & realB)
{
        char aBuf[100] = {'\0'}, bBuf[100] = {'\0'}, cBuf[100] = {'\0'}, dBuf[100] = {'\0'}, realABuf[100] = {'\0'}, realBBuf[100] = {'\0'};
        int item = 0, j0 = 0, j1 = 0, j2 = 0, j3 = 0, rj0 = 0, rj1 = 0, i, aTag, bTag;
        ...
        a = atoi(aBuf);
        realA = aTag == 1 ? atoi(realABuf) : -1;
        b = atoi(bBuf);
        realB = bTag == 1 ? atoi(realBBuf) : -1;
        c = atoi(cBuf);
        d = atoi(dBuf);
}

void delta2psl(string st0)
{
        ifstream in;
        ofstream out;
        string st1, buf, align;
        int i, j, sp, b, targetID, sourceID, targetSize, sourceSize, targetStart, targetEnd, sourceStart, sourceEnd, sourceGap, targetGap, realSourceID, realTargetID, dummyID;
        char fr;
        vector<Segment> seg;
        Segment s;

        in.open(st0.c_str());
	st1 = st0.substr(0, st0.find('.', st0.find('.', 0) + 1)).append(".psl");
        out.open(st1.c_str());
        ...
        if(in.is_open())
        {
                ...
                while(in.good())
                {
                        align.clear(); seg.clear(); sourceGap = targetGap = 0;
                        getline(in, buf);
                        if(buf[0] == '\0') break;

                        if(buf[0] == '>')
                        {
                                parseDelta(buf, targetID, sourceID, targetSize, sourceSize, realTargetID, realSourceID);
                                getline(in, buf);
                        }
                        parseDelta(buf, targetStart, targetEnd, sourceStart, sourceEnd, dummyID, dummyID);
                        ...
                        for(i = align.size(); i < (sourceEnd - sourceStart + 1) + targetGap; i ++)
                                align.push_back('M');

                        //calculate seg
                        for(i = 0, j = 0; i < align.size(); i ++)
                        {
                                if(align[i] != 'I')
                                        j ++;
                                if(i == 0 && align[i] == 'M')
                                {
                                        s.sourceStart = sourceStart - 1 + j;
                                        s.targetStart = s.size = -1;
                                        seg.push_back(s);
                                }
                                if(i - 1 >= 0 && (align[i - 1] == 'I' || align[i - 1] == 'D') && align[i] == 'M')
                                {
                                        s.sourceStart = sourceStart - 1 + j;
                                        s.targetStart = s.size = -1;
                                        seg.push_back(s);
                        ...
                        for(i = 0, j = 0, sp = 0; i < align.size(); i ++)
                        {
                                if(align[i] != 'D')
                                        j ++;
                                if(i == 0 && align[i] == 'M')
                                {
                                        seg[sp ++].targetStart = targetStart - 1 + j;
                                }
                                if(i - 1 >= 0 && (align[i - 1] == 'I' || align[i - 1] == 'D') && align[i] == 'M')
                                {
                                        seg[sp ++].targetStart = targetStart - 1 + j;
                        ...
                        for(i = 0, j = 0, sp = 0; i < align.size(); i ++)
                        {
                                if(align[i] == 'M')
                                        j ++;
                                if(i + 1 < align.size() && align[i] == 'M' && (align[i + 1] == 'I' || align[i + 1] == 'D'))
                                {
                                        seg[sp ++].size = j;
                                        j = 0;
                                }
                                if(i == align.size() - 1 && align[i] == 'M')
                                        seg[sp].size = j;
                        }

                        //write out one psl line
                        sourceStart --;
                        targetStart --;
                        ...
                        out << "NA	NA	NA	NA	NA	" << sourceGap << "	NA	" << targetGap << "	" << fr << "	";
                        if(realSourceID != -1)
                                out << sourceID << "." << realSourceID << "	";
                        else
                                out << sourceID << "	";
                        out << sourceSize << "	" << sourceStart << "	" << sourceEnd << "	";
                        if(realTargetID != -1)
                                out << targetID << "." << realTargetID << "	";
                        else
                                out << targetID << "	";
                        out << targetSize << "	" << targetStart << "	" << targetEnd << "	NA	";
                        for(i = 0; i < seg.size(); i ++)
                                out << seg[i].size << ",";
                        out << "	";
                        for(i = 0; i < seg.size(); i ++)
                                out << seg[i].sourceStart << ",";
                        out << "	";
                        for(i = 0; i < seg.size(); i ++)
                                out << seg[i].targetStart << ",";
                        out << endl;
                }
        }
        else
...
void ref1(int numth=8)
{
	...
	string numth_str = string(itoa(numth));
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        for(i = 0; i < numChromosomes; i ++)
	{
		s = "pblat tmp/_extended_contigs." + itoa(i) + ".fa tmp/_short_initial_contigs." + itoa(i) + ".fa -noHead tmp/_short_initial_contigs_extended_contigs." + itoa(i) + ".psl -threads="+numth_str+" > blat_doc.txt 2> blat_doc.txt";
		if(system(s.c_str()) != 0)
		{
			...
			if(system(s.c_str()) != 0) {cout << "BLAT CALL FAILED!" << endl; exit(-1);}
		}
	}
...
void ref2(ofstream & e, ofstream & r, int numth=8)
{
        int numChromosomes, i, j, k, seqID;
        string s;
        vector<vector<char> > initContigs, extdContigs;
	...
	string numth_str = string(itoa(numth));
        ...
        for(i = 0; i < numChromosomes; i ++)
        {
                s = "pblat tmp/_post_extended_contigs." + itoa(i) + ".fa tmp/_post_initial_contigs." + itoa(i) + ".fa -noHead tmp/_initial_contigs_extended_contigs." + itoa(i) + ".psl -threads="+numth_str+" > blat_doc.txt 2> blat_doc.txt";
                if(system(s.c_str()) != 0)
		{
...
void refinement(ofstream & e, ofstream & r, int numth=8)
{
	ref1(numth);
	ref2(e, r, numth);
}

void refinement(ofstream & e, ofstream & r, int fastMap, int uniqueExtension, int numChromosomes, int numth=8)
{
        int i, j, k, seqID;
        string s, s0;
        vector<vector<char> > initContigs, extdContigs;
...
//NUCMER
		for(i = 0; i < numChromosomes; i ++)
		{
			s = "nucmer tmp/_extended_contigs." + itoa(i) + ".fa tmp/_short_initial_contigs." + itoa(i) + ".fa -p tmp/_short_initial_contigs_extended_contigs." + itoa(i) + " > nucmer_doc.txt 2> nucmer_doc.txt";
			if(system(s.c_str()) != 0) 
			{
				s = "touch tmp/_short_initial_contigs_extended_contigs." + itoa(i) + ".delta";
				system(s.c_str());
			}
			s = "tmp/_short_initial_contigs_extended_contigs." + itoa(i) + ".delta"; 
			delta2psl(s);
		}
	}
	else
	{
	        for(i = 0; i < numChromosomes; i ++)
	        {
	                //IP: delete -fastMap option that limits qseq to 5000bp
			//s = "pblat tmp/_extended_contigs." + itoa(i) + ".fa tmp/_short_initial_contigs." + itoa(i) + ".fa -noHead tmp/_short_initial_contigs_extended_contigs." + itoa(i) + ".psl -fastMap -threads=8 > blat_doc.txt 2> blat_doc.txt";
	                s = "pblat tmp/_extended_contigs." + itoa(i) + ".fa tmp/_short_initial_contigs." + itoa(i) + ".fa -noHead tmp/_short_initial_contigs_extended_contigs." + itoa(i) + ".psl -threads="+numth_str+" > blat_doc.txt 2> blat_doc.txt";
			if(system(s.c_str()) != 0)
			{
				//s = "blat tmp/_extended_contigs." + itoa(i) + ".fa tmp/_short_initial_contigs." + itoa(i) + ".fa -noHead tmp/_short_initial_contigs_extended_contigs." + itoa(i) + ".psl -fastMap > blat_doc.txt 2> blat_doc.txt";
				s = "blat tmp/_extended_contigs." + itoa(i) + ".fa tmp/_short_initial_contigs." + itoa(i) + ".fa -noHead tmp/_short_initial_contigs_extended_contigs." + itoa(i) + ".psl > blat_doc.txt 2> blat_doc.txt";
				if(system(s.c_str()) != 0) {cout << "BLAT CALL FAILED!" << endl; exit(-1);}
			}
	        }
...
void * task0(void * arg)
{
	...
	ins.iterativeMap;
	// Modified
	string numth_str = string(itoa(ins.numth));
	...
	if(ins.iterativeMap == 1)
	{
		for(chromosomeID = 0; chromosomeID < ins.numChromosomes; chromosomeID ++)
		{
			...
			command = "bowtie2 -f --no-mixed -k 5 -p "+numth_str+" --local --mp 3,1 --rdg 2,1 --rfg 2,1 --score-min G,5,2 -I " + distanceLowStr.str() + " -X " + distanceHighStr.str() + " --no-discordant -x tmp/_genome." + itoa(chromosomeID) + " -1 tmp/_reads_1.fa -2 tmp/_reads_2.fa --reorder > tmp/_reads_genome." + itoa(chromosomeID) + ".bowtie 2> bowtie_doc.txt";
			system(command.c_str());
		}
	}
	else
	{
		...
		system(command.c_str());
		command = "bowtie2 -f --no-mixed -k 5 -p "+numth_str+" --local --mp 3,1 --rdg 2,1 --rfg 2,1 --score-min G,5,2 -I " + distanceLowStr.str() + " -X " + distanceHighStr.str() + " --no-discordant -x tmp/_genome -1 tmp/_reads_1.fa -2 tmp/_reads_2.fa --reorder > tmp/_reads_genome.bowtie 2> bowtie_doc.txt";
		system(command.c_str());
...
//NUCMER
		for(chromosomeID = 0; chromosomeID < ins.numChromosomes; chromosomeID ++)
		{
			command = "nucmer tmp/_genome." + itoa(chromosomeID) + ".fa tmp/_contigs.fa -p tmp/_contigs_genome." + itoa(chromosomeID) + " > nucmer_doc.txt 2> nucmer_doc.txt";
			if(system(command.c_str()) != 0) 
			{
				command = "touch tmp/_contigs_genome." + itoa(chromosomeID) + ".delta";
				system(command.c_str());
			}
			command = "tmp/_contigs_genome." + itoa(chromosomeID) + ".delta";
			delta2psl(command);
		}
	}
	else
	{
		for(chromosomeID = 0; chromosomeID < ins.numChromosomes; chromosomeID ++)
		{
			//IP: delete -fastMap option with limit on qseq length
			//command = "pblat tmp/_genome." + itoa(chromosomeID) + ".fa tmp/_contigs.fa -noHead tmp/_contigs_genome." + itoa(chromosomeID) + ".psl -fastMap -threads=8 > blat_doc.txt 2> blat_doc.txt";
			command = "pblat tmp/_genome." + itoa(chromosomeID) + ".fa tmp/_contigs.fa -noHead tmp/_contigs_genome." + itoa(chromosomeID) + ".psl -threads="+numth_str+" > blat_doc.txt 2> blat_doc.txt";
			
			if(system(command.c_str()) != 0)
			{
				//command = "blat tmp/_genome." + itoa(chromosomeID) + ".fa tmp/_contigs.fa -noHead tmp/_contigs_genome." + itoa(chromosomeID) + ".psl -fastMap > blat_doc.txt 2> blat_doc.txt";
				command = "blat tmp/_genome." + itoa(chromosomeID) + ".fa tmp/_contigs.fa -noHead tmp/_contigs_genome." + itoa(chromosomeID) + ".psl > blat_doc.txt 2> blat_doc.txt";
				
				if(system(command.c_str()) != 0) {cout << "BLAT CALL FAILED!" << endl; exit(-1);}
			}
		}
	}
}

void nonParallelMap(int distanceLow, int distanceHigh, int numChromosomes, int fastMap, int iterativeMap, int numth=8)
{
        string command, s;
        stringstream distanceLowStr, distanceHighStr;
	int chromosomeID;
...
//	              system(command.c_str());
			...
			//command = "pblat tmp/_genome." + itoa(chromosomeID) + ".fa tmp/_contigs.fa -noHead tmp/_contigs_genome." + itoa(chromosomeID) + ".psl -fastMap -threads=8 > blat_doc.txt 2> blat_doc.txt";// -minIdentity=50 -q=dnax -t=dnax
			command = "pblat tmp/_genome." + itoa(chromosomeID) + ".fa tmp/_contigs.fa -noHead tmp/_contigs_genome." + itoa(chromosomeID) + ".psl -threads="+numth_str+" > blat_doc.txt 2> blat_doc.txt";// -minIdentity=50 -q=dnax -t=dnax
...
void parallelMap(int distanceLow, int distanceHigh, int numChromosomes, int fastMap, int iterativeMap, int numth=8)
{
	pthread_t t0, t1;
	Insert ins;
	...
	ins.numth = numth;
	if(pthread_create(&t0, NULL, task0, &ins) != 0) {nonParallelMap(distanceLow, distanceHigh, numChromosomes, fastMap, iterativeMap, numth); return;}
	if(pthread_create(&t1, NULL, task1, &ins) != 0) {nonParallelMap(distanceLow, distanceHigh, numChromosomes, fastMap, iterativeMap, numth); return;}
...
void makeAlignment(int distanceLow, int distanceHigh, string id, int fastMap, int numth = 8)
{
	string command, s;
	...
	system(command.c_str());
	command = "bowtie2 -f --no-mixed -k 1 -p "+numth_str+" -I " + itoa(distanceLow) + " -X " + itoa(distanceHigh) + " --no-discordant -x tmp/_" + id + "_contigs -1 tmp/_reads_1.fa -2 tmp/_reads_2.fa --reorder > tmp/_reads_" + id + "_contigs.bowtie 2> bowtie_doc.txt";

	system(command.c_str());
	if(fastMap == 1)
	{
//NUCMER
		command = "nucmer tmp/_genome.fa tmp/_" + id + "_contigs.fa -p tmp/_" + id  + "_contigs_genome.delta > nucmer_doc.txt 2> nucmer_doc.txt";
		if(system(command.c_str()) != 0) 
		{
			command = "touch tmp/_" + id  + "_contigs_genome.delta";
			system(command.c_str());
		}
		command = "tmp/_" + id  + "_contigs_genome.delta";
		delta2psl(command);
	}
	else
	{
		//command = "pblat tmp/_genome.fa tmp/_" + id + "_contigs.fa -noHead tmp/_" + id  + "_contigs_genome.psl -fastMap -threads=8 > blat_doc.txt 2> blat_doc.txt";
		command = "pblat tmp/_genome.fa tmp/_" + id + "_contigs.fa -noHead tmp/_" + id  + "_contigs_genome.psl -threads="+numth_str+" > blat_doc.txt 2> blat_doc.txt";
		
		if(system(command.c_str()) != 0)
		{
			//command = "blat tmp/_genome.fa tmp/_" + id + "_contigs.fa -noHead tmp/_" + id  + "_contigs_genome.psl -fastMap > blat_doc.txt 2> blat_doc.txt";
			command = "blat tmp/_genome.fa tmp/_" + id + "_contigs.fa -noHead tmp/_" + id  + "_contigs_genome.psl > blat_doc.txt 2> blat_doc.txt";
			if(system(command.c_str()) != 0) {cout << "BLAT CALL FAILED!" << endl; exit(-1);}
		}
	}
}
...
void removeMisassembly(string file, int distanceLow, int distanceHigh, string id, int coverage, int fastMap, int numth = 8)
{
	ifstream c;
	string s;
	...
	makeAlignment(distanceLow, distanceHigh, id, fastMap, numth);
...
void print()
{
        ...
        cout << "--fastMap calls NUCMER to make fast but less sensitive and accurate contig alignment instead of BLAT (default: none)" << endl;
...
void testAligners(int tagFastMap)
{
	if(system("bowtie2 -h > bowtie_doc.txt 2> bowtie_doc.txt") != 0) {cout << "BOWTIE2 CALL FAILED!" << endl; exit(-1);}
	if(tagFastMap == 0)	
	{
//		if(system("pblat > blat_doc.txt 2> blat_doc.txt") != 0 && system("blat -h > blat_doc.txt 2> blat_doc.txt") != 0) {cout << "BLAT CALL FAILED!" << endl; exit(-1);}
// BLAT check is left for later when it is running
	}
	else
	{
		if(system("nucmer -h > nucmer_doc.txt 2> nucmer_doc.txt") != 0) {cout << "NUCMER CALL FAILED!" << endl; exit(-1);}
	}
}
...
int main(int argc, char * argv[])   
{
	...
	if(tagResume == 0)
	{
		...
		parallelMap(distanceLow, distanceHigh, numChromosomes, tagFastMap, tagIterativeMap, numth);
		endAlign = time(NULL);
	...
	if(tagMisassemblyRemoval == 1)
	{
		removeMisassembly(ext, distanceLow, distanceHigh, "extended", coverage, tagFastMap, numth);
		removeMisassembly(rmn, distanceLow, distanceHigh, "remaining", coverage, tagFastMap, numth);
